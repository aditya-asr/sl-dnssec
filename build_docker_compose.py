...
for client in client_data:
    ...
    # os.mkdir("build/" + client["name"] + "/OQS-bind")
    # os.system("cp -r ../../CLionProjects/OQS-bind-latest/* build/" + client["name"] + "/OQS-bind")
...
for resolver in resolver_data:
    ...
    os.mkdir("build/" + resolver["name"] + "/sl_dnssec")
    ...
    os.system("cp -r daemon/* build/" + resolver["name"] + "/sl_dnssec")
    # os.mkdir("build/" + resolver["name"] + "/OQS-bind")
    # os.system("cp -r ../../CLionProjects/OQS-bind-latest/* build/" + resolver["name"] + "/OQS-bind")
    with open("build/" + resolver["name"] + "/Dockerfile", "a") as f:
        if BYPASS:
            f.write(
                "CMD /setup_files/install_trust_anchor.bash && rm -rf /dsset/* && iptables -A INPUT -p ip -j NFQUEUE --queue-num 0 && iptables -A OUTPUT -p ip -j NFQUEUE --queue-num 0 && ifconfig && ./sl_dnssec/daemon ${LISTENIP} --bypass & named -g -d 3\n")
        else:
            f.write(
                "CMD /setup_files/install_trust_anchor.bash && rm -rf /dsset/* && iptables -A INPUT -p ip -j NFQUEUE --queue-num 0 && iptables -A OUTPUT -p ip -j NFQUEUE --queue-num 0 && ifconfig && ./sl_dnssec/daemon ${LISTENIP} --maxudp " + str(
...
for name_server in ns_data:
    ...
    os.mkdir("build/" + name_server["name"] + "/sl_dnssec")
    ...
    os.system("cp -r daemon/* build/" + name_server["name"] + "/sl_dnssec")
    # os.mkdir("build/" + name_server["name"] + "/OQS-bind")
    # os.system("cp -r ../../CLionProjects/OQS-bind-latest/* build/" + name_server["name"] + "/OQS-bind")
    ...
    if name_server["leaf"] == "true":
        ...
        if BYPASS:
            docker_file.write("CMD /setup_files/move_ds.bash " + name_server[
                "zone"] + " && iptables -A INPUT -p ip -j NFQUEUE --queue-num 0 && iptables -A OUTPUT -p ip -j NFQUEUE --queue-num 0 && ifconfig && cd / && ./sl_dnssec/daemon ${LISTENIP} --bypass & named -d 3 && /bin/bash\n")
        else:
            docker_file.write("CMD /setup_files/move_ds.bash " + name_server[
                "zone"] + " && iptables -A INPUT -p ip -j NFQUEUE --queue-num 0 && iptables -A OUTPUT -p ip -j NFQUEUE --queue-num 0 && ifconfig && cd / && ./sl_dnssec/daemon ${LISTENIP} --maxudp " + str(
        ...
        if BYPASS:
            cmd_str += "&& if [ ! -f /usr/local/etc/bind/zones/" + name_server[
                "file"] + ".signed ]; then dnssec-signzone -o " + out + " -N INCREMENT -t -S -K /usr/local/etc/bind/zones db." + zone + ";fi && /setup_files/move_ds.bash . && iptables -A INPUT -p ip -j NFQUEUE --queue-num 0 && iptables -A OUTPUT -p ip -j NFQUEUE --queue-num 0 && ifconfig && cd / && ./sl_dnssec/daemon ${LISTENIP} --bypass & named -g -d 3\n"
        else:
            cmd_str += "&& if [ ! -f /usr/local/etc/bind/zones/" + name_server[
                "file"] + ".signed ]; then dnssec-signzone -o " + out + " -N INCREMENT -t -S -K /usr/local/etc/bind/zones db." + zone + ";fi && /setup_files/move_ds.bash . && iptables -A INPUT -p ip -j NFQUEUE --queue-num 0 && iptables -A OUTPUT -p ip -j NFQUEUE --queue-num 0 && ifconfig && cd / && ./sl_dnssec/daemon ${LISTENIP} --maxudp " + str(
                maxudp) + " & named -g -d 3\n"
