...
int
create_rr_mac(ResourceRecord **out, char *name, unsigned char *name_bytes, size_t name_byte_len, uint16_t type,
              uint16_t clas, uint32_t ttl, uint16_t rdsize, unsigned char *rdata, ResourceRecord *msg_rr,
    ...
    if (rr->type == 46) {
        ...
        int num_msg_bytes = 0;
        num_msg_bytes += i_copy;             // num RRSIG->RDATA \ Signature

//        printf("\ni_copy: %d\n", i_copy);

        unsigned char *rrout;
        size_t rr_outlen;
        rr_to_bytes(msg_rr, &rrout, &rr_outlen);
//        printf("\nrr_len: %d\n", rr_outlen);

        num_msg_bytes += rr_outlen;
//        printf("\nnum_bytes_to_mac: %d\n", num_msg_bytes);

        unsigned char *data = malloc(num_msg_bytes);
        rdata[2] = KYBER_512_ALG;                  // Switch to KEM alg id

        /*
         * Key tag currently is of the ZSK. Deployments will need to set the Key Tag of ZKK here
         * */

        memcpy(data, rdata, i_copy);        //  RRSIG->RDATA \ Signature
        memcpy(data + i_copy, rrout, rr_outlen);        //   RRSIG->RDATA \ Signature || RR

//        printf("\nRRSIG->RDATA except SIG: ");
//        for (int i = 0; i < i_copy; i++)
//            printf("%02x", rdata[i]);
//
//        printf("\nRR: ");
//        for (int i = 0; i < rr_outlen; i++)
//            printf("%02x", rrout[i]);
//
//        printf("\nMAC msg: ");
...
//            printf("%02x", data[i]);
        ...
        unsigned char *key = derive_hkdf_key(shared_secret, OQS_KEM_kyber_512_length_shared_secret);
        unsigned char *result = gen_hmac(key, HASH_SIZE, data, num_msg_bytes);

        rr->rdsize = i_copy + HASH_SIZE;
        rr->rdata = malloc(rr->rdsize);
        ...
        memcpy(rr->rdata, rdata, i_copy);
        memcpy((rr->rdata) + i_copy, result, HASH_SIZE);
        *out = rr;
        return 0;
...
int
verify_rr_mac(char *name, unsigned char *name_bytes, size_t name_byte_len, uint16_t type,
              uint16_t clas, uint32_t ttl, uint16_t rdsize, unsigned char *rdata, ResourceRecord *msg_rr,
...
//            printf("%02x", data[i]);
        ...
        unsigned char *key = derive_hkdf_key(shared_secret_e, OQS_KEM_kyber_512_length_shared_secret);

        unsigned char *result = gen_hmac(key, HASH_SIZE, data, num_msg_bytes);

        char *mac = malloc(HASH_SIZE);
        memcpy(mac, rdata + i_copy, HASH_SIZE);        //  RRSIG->RDATA->Sig (MAC)
...
int get_alg_sig_pk_size(uint16_t type, unsigned char *rdata) {
    if (type == 46) {
        ...
        } else if (SIG_ALG == FALCON_1024_ALG) {
            printf("\nFALCON 1024 RRSIG RR Found.");
            SIG_SIZE = FALCON_1024_SIG_SIZE;
        } else if (SIG_ALG == DILITHIUM5_ALG) {
            printf("\nDILITHIUM5 RRSIG RR Found.");
            SIG_SIZE = DILITHIUM5_SIG_SIZE;
        }
        return SIG_SIZE;
    } else if (type == 48) {
        const char PK_ALG = rdata[3];;
        ...
        } else if (PK_ALG == FALCON_1024_ALG) {
            printf("\nFALCON 1024 DNSKEY RR Found.");
            PK_SIZE = FALCON_1024_PK_SIZE;
        } else if (PK_ALG == DILITHIUM5_ALG) {
            printf("\nDILITHIUM 5 DNSKEY RR Found.");
            PK_SIZE = DILITHIUM5_PK_SIZE;
        }
        return PK_SIZE;
    }
    return -1;
