...
bool internal_send(int fd, unsigned char *bytes, size_t byte_len,
    ...
    for (int i = 0; i < msg->arcount; i++) {
        ResourceRecord *rr = msg->additional_section[i];
        if (rr->type == DNSKEY && rr->rdata[3] == KYBER_512_ALG) {
            printf("\nKEM CT found! Saving...");
            clone_rr(rr, &(ci->rr_ct));
        }
    }
...
void responding_thread_end(struct iphdr *iphdr, void *transport_hdr, bool is_tcp,
                           ...
                           char *qname, ResourceRecord *rr_ct) {
    ...
    if (recvd_len <= MAXUDP) {
        ...
        if (rr_ct != NULL) {
            printf("\nReplacing Sigs with MACs...\n");

            uint8_t ciphertext[OQS_KEM_kyber_512_length_ciphertext];
            memcpy(ciphertext, (rr_ct->rdata + 4), OQS_KEM_kyber_512_length_ciphertext);

            uint8_t shared_secret_d[OQS_KEM_kyber_512_length_shared_secret];
            kyber_decap(shared_secret_d, ciphertext, KYBER_512_ZSK_PRIV);

            DNSMessage *m;
            clone_dnsmessage(recvd_msg, &m);

            Question **question_section = malloc(sizeof(Question * ) * m->qdcount);

            for (uint16_t i = 0; i < m->qdcount; i++) {
                clone_question(m->question_section[i], question_section + i);
            }
            ...
            ResourceRecord *rr_to_mac;                  // first find rr to calculate mac for
            for (int i = 0; i < m->ancount; i++) {
                rr_to_mac = m->answers_section[i];
                if (rr_to_mac->type != RRSIG)
                    break;
            }
            ...
            for (int i = 0; i < m->ancount; i++) {
                ...
                if (rr->type == RRSIG) {
                    ResourceRecord *rr_fragment;
                    create_rr_mac(&rr_fragment, rr->name, rr->name_bytes,
                                  rr->name_byte_len, rr->type, rr->clas, rr->ttl,
                                  rr->rdsize, rr->rdata, rr_to_mac, shared_secret_d);
                    clone_rr(rr_fragment, answers_section + ancount);
                    ancount++;
                } else {
                    clone_rr(rr, answers_section + ancount);
            ...
            for (int i = 0; i < m->nscount; i++) {
                rr_to_mac = m->authoritative_section[i];
                if (rr_to_mac->type != RRSIG)
                    break;
            }
            ...
            for (int i = 0; i < m->arcount; i++) {
                rr_to_mac = m->additional_section[i];
                if (rr_to_mac->type != RRSIG && rr_to_mac->type != OPT)
                    break;
            }
...
uint32_t process_dns_message(struct nfq_q_handle *qh, uint32_t id,
    ...
    if (is_internal_packet(iphdr)) {
        ...
        if (handle_internal_packet(qh, id, iphdr, question_hash_port, outbuff, &outbuff_len)
            ...
            responding_thread_end(ci->iphdr, ci->transport_header, ci->is_tcp,
                                  ...
                                  ci->fd, ci->frag_num, ci->qname, ci->rr_ct);
        } else {
            return NF_ACCEPT;
        }
        return 0xFFFF;
    ...
    if (is_query(msg)) {
        ...
        if (saddr == our_addr && dport == 53) {
            if (is_kem_dnsmessage(msg)) {
                printf("Resolver : Send SL-DNSSEC Query \n");
                if (DEBUG)
                    dnsmessage_to_string(msg);
                return NF_ACCEPT;
            }
            ...
            if (sl_dnssec && (msg->question_section[0]->qtype) == 1) {

                // Only start SL-DNSSEC protocol for QTYPE A queries directed to example ANS
                printf("\n *** SL-DNSSEC *** \n");

                uint8_t shared_secret_e[OQS_KEM_kyber_512_length_shared_secret];
                uint8_t ciphertext[OQS_KEM_kyber_512_length_ciphertext];
                kyber_encap(ciphertext, shared_secret_e, KYBER_512_ZSK_PUB);

                // Clone OPT and modify it into a DNSKEY record
                // Here, rr -> name = ".", but deployments will probably use zone apex "example."
                ResourceRecord *rr;
                clone_rr(msg->additional_section[0], &rr);
                rr->type = 48;  // DNSKEY
                rr->clas = 1;
                rr->ttl = 604800;
                rr->rdsize = OQS_KEM_kyber_512_length_ciphertext + 4;
                free(rr->rdata);
                rr->rdata = malloc(sizeof(unsigned char) * rr->rdsize);

                // Flags, Protocol, Algorithm
                const unsigned char bytearr[] = {0x01, 0x02, 0x03, KYBER_512_ALG};

                memcpy(rr->rdata, bytearr, 4);
                memcpy((rr->rdata) + 4, ciphertext, (rr->rdsize) - 4);
                ...
                *id = msg->identification;
                ...
                if (!hashmap_get(requester_state, id, sizeof(uint16_t), &out)) {
                    ...
                    memcpy(store->shared_secret_e, shared_secret_e, OQS_KEM_kyber_512_length_shared_secret);
                    hashmap_set(requester_state, id, sizeof(uint16_t), (uintptr_t) store);
                }

                // copy all records and send the DNS query
                ResourceRecord **additional_section = malloc(sizeof(ResourceRecord * ) * 2);
                clone_rr(rr, additional_section);
                clone_rr(msg->additional_section[0], additional_section + 1);

                Question **question_section = malloc(sizeof(Question * ));
                clone_question(msg->question_section[0], question_section);

                DNSMessage *tmp;
                create_dnsmessage(&tmp, msg->identification, msg->flags, 1, 0, 0, 2,
                                  question_section, NULL, NULL, additional_section);

                send_dns_messsge(tmp, iphdr, transport_header, is_tcp, 0);
                return NF_DROP;
            }
            ...
            *id = msg->identification;
            ...
            if (!hashmap_get(requester_state, id, sizeof(uint16_t), &out)) {
                ...
                if (MODE == 2) {
                    store->num_required_frags = 1;              // default min = 1
                    if (ALG == 0) {                             // FALCON-512
                        ...
                        if (msg->question_section[0]->qtype == DNSKEY)
                            ...
                            store->num_required_frags = 8;
                        } else
                            store->num_required_frags = 3;
                    } else if (ALG == 2) {              // SPHINCS+ LEVEL 1
                        if (msg->question_section[0]->qtype == DNSKEY)
                            ...
                            store->num_required_frags = 7;
                    } else if (ALG == 3) {                // FALCON-1024
                        if (msg->question_section[0]->qtype == DNSKEY)
                            store->num_required_frags = 6;
                        else if (msg->question_section[0]->qtype == 1) {
                            store->num_required_frags = 4;
                        } else
                            store->num_required_frags = 2;
                    } else if (ALG == 4) {                // Dilithium5
                        if (msg->question_section[0]->qtype == DNSKEY)
                            store->num_required_frags = 14;
                        else if (msg->question_section[0]->qtype == 1) {
                            store->num_required_frags = 14;
                        } else
                            store->num_required_frags = 5;
                    }
            ...
            if (!is_resolver) {
                if (is_kem_dnsmessage(msg)) {
                    printf("* SL-DNSSEC Query Detected * \n");
                }
        ...
        if (daddr == our_addr && sport == 53) {
            ...
            if (is_kem_dnsmessage(msg)) {
                ...
                if ((msg->flags & 0x0040U) != 0) {
                    printf("\nZ flag is set. Accept and send to BIND9.");
                    return NF_ACCEPT;
                }
                ...
                *id = msg->identification;
                ...
                if (hashmap_get(requester_state, id, sizeof(uint16_t), (uintptr_t * ) & store)) {
                    memcpy(shared_secret_e, store->shared_secret_e, OQS_KEM_kyber_512_length_shared_secret);
                }
                ...
                for (int i = 0; i < m->ancount; i++) {
                    ...
                    if (rr->type == RRSIG) {
                        if (!verify_rr_mac(rr->name, rr->name_bytes,
                                           rr->name_byte_len, rr->type, rr->clas, rr->ttl,
                                           rr->rdsize, rr->rdata, rr_to_mac, shared_secret_e))
                            is_verified = false;
                    }
                }
                ...
                if (is_verified) {
                    printf("\n\nSL-DNSSEC Verify: Ok");
                    printf("\nSetting Z Flag...\n");
                    msg->flags = msg->flags | 0x0040U;    // Set Z
                    send_dns_messsge(msg, iphdr, transport_header, is_tcp, 0);
                    return NF_DROP;
                } else
                    printf("\nSL-DNSSEC Verify: Fail");
                return NF_ACCEPT;
            }
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--is_resolver") == 0) {
            ...
            if (strcmp(argv[i], "FALCON512") == 0)
                ...
                ALG = 1;
            else if (strcmp(argv[i], "SPHINCS+-SHA256-128S") == 0)
                ALG = 2;
            else if (strcmp(argv[i], "FALCON1024") == 0)
                ALG = 3;
            else if (strcmp(argv[i], "DILITHIUM5") == 0)
                ALG = 4;
